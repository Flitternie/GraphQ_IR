...
# for val in ${domains[@]}; do
    ...
    # python -m IR_unified.preprocess_overnight --input_dir ./data/overnight/dataset/ --output_dir ./exp_files_new/UIR/overnight/$val/ --model_name_or_path ./bart-base/ --domain $val
    # srun -A priority -p Big -n 1 --gres=gpu:1 --job-name=d_$val bash -c "python -m torch.distributed.launch --nproc_per_node=1 -m train --input_dir ./exp_files_new/UIR/overnight/$val/ --output_dir ./exp_results_new/UIR/overnight/$val/ --save_dir ./exp_results_new/UIR/overnight/$val/ --model_name_or_path ./bart-base/ --batch_size 32 --port 12375 --num_train_epochs 50 --mode ir --early_stopping 10" 1> ./logging/$val.log 2>/dev/null &
